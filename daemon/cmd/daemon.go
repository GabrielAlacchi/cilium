...
func NewDaemon(ctx context.Context, epMgr *endpointmanager.EndpointManager, dp datapath.Datapath) (*Daemon, *endpointRestoreState, error) {
	...
	if option.Config.EnableHostReachableServices {
		isKind, err := k8s.IsProviderKind(k8s.Client(), nodeTypes.GetName())
		if err == nil {
			runsOnKind = isKind
		} else {
			log.WithError(err).
				Warnf("Failed to retrieve k8s node provider. If running on Kind, socket-based LB (--%s) will not work.",
